// Kakao Local REST 연동: 주소지오코딩 + 키워드/카테고리 검색
...
// 키워드 장소검색 (예: '카페', '분식')
export async function searchKeyword({ query, x, y, radius=700 }){
    if(!KAKAO) return { total: 0, items: [] }
...
// 카테고리 그룹 코드 검색 (예: CE7=카페, FD6=음식점, CS2=편의점)
export async function searchCategory({ code, x, y, radius=700 }){
    if(!KAKAO) return { total: 0, items: [] }
    const r = await client.get('/search/category.json', {
        params: { category_group_code: code, x, y, radius, sort: 'distance' }
    })
    const items = r.data?.documents || []
    return { total: Number(r.data?.meta?.total_count||items.length), items }
}
